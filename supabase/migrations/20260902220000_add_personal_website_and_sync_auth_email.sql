-- Add a public personal website while keeping sign-in email changes under Supabase Auth verification.

ALTER TABLE public.crm_distributors
ADD COLUMN IF NOT EXISTS website_url TEXT;

ALTER TABLE public.crm_distributors
DROP CONSTRAINT IF EXISTS crm_distributors_website_url_valid;

ALTER TABLE public.crm_distributors
ADD CONSTRAINT crm_distributors_website_url_valid
CHECK (
...
AS $$
  SELECT d.id, d.slug, d.referral_code, d.display_name, d.title, d.bio,
    ...
    d.website_url, COALESCE(d.purchase_links, '{}'::jsonb)
  FROM public.crm_distributors d
  WHERE d.active AND d.accepting_leads
  ORDER BY d.display_name;
$$;

REVOKE ALL ON FUNCTION public.get_public_crm_distributors() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_public_crm_distributors() TO anon, authenticated;

CREATE OR REPLACE FUNCTION public.crm_update_distributor_profile(p_distributor_id UUID, p_payload JSONB)
...
DECLARE
  ...
  normalized_website TEXT := nullif(trim(COALESCE(p_payload ->> 'websiteUrl', '')), '');
...
BEGIN
  ...
  IF NOT ((SELECT public.crm_is_admin()) OR p_distributor_id = (SELECT public.crm_current_distributor_id())) THEN RAISE EXCEPTION 'Not authorized'; END IF;
  ...
  IF normalized_website IS NOT NULL AND (char_length(normalized_website) > 500 OR normalized_website !~ '^https?://[^[:space:]]+$') THEN RAISE EXCEPTION 'Invalid website URL'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM jsonb_each(normalized_purchase_links) link WHERE char_length(link.key) NOT BETWEEN 1 AND 80 OR jsonb_typeof(link.value) <> 'string' OR char_length(trim(link.value #>> '{}')) > 2000 OR trim(link.value #>> '{}') !~ '^https?://[^[:space:]]+$') THEN RAISE EXCEPTION 'Invalid purchase link URL'; END IF;

  UPDATE public.crm_distributors SET
    display_name = normalized_name, title = normalized_title, bio = nullif(normalized_bio, ''),
    phone = normalized_phone, instagram_url = normalized_instagram, website_url = normalized_website,
    avatar_url = COALESCE(normalized_avatar, avatar_url), regions = normalized_regions,
    languages = normalized_languages,
    accepting_leads = COALESCE((p_payload ->> 'acceptingLeads')::BOOLEAN, accepting_leads),
    purchase_links = normalized_purchase_links
  WHERE id = p_distributor_id RETURNING * INTO result;
  IF result.id IS NULL THEN RAISE EXCEPTION 'Distributor not found'; END IF;
  RETURN result;
END;
$$;

REVOKE ALL ON FUNCTION public.crm_update_distributor_profile(UUID, JSONB) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.crm_update_distributor_profile(UUID, JSONB) TO authenticated;

CREATE SCHEMA IF NOT EXISTS private;
REVOKE ALL ON SCHEMA private FROM PUBLIC, anon, authenticated;

CREATE OR REPLACE FUNCTION private.crm_sync_confirmed_auth_email()
RETURNS TRIGGER
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp
AS $$
BEGIN
...
REVOKE ALL ON FUNCTION private.crm_sync_confirmed_auth_email() FROM PUBLIC, anon, authenticated;
DROP TRIGGER IF EXISTS crm_sync_confirmed_auth_email ON auth.users;
CREATE TRIGGER crm_sync_confirmed_auth_email
AFTER UPDATE OF email ON auth.users
FOR EACH ROW EXECUTE FUNCTION private.crm_sync_confirmed_auth_email();
