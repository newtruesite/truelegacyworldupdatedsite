...
export default function AppSharePage(){
  ...
  return <main className="min-h-screen bg-[#05091a] px-4 pb-28 pt-[max(22px,env(safe-area-inset-top))] text-white sm:px-6"><SEO title="True Legacy Share Center" description="Choose and share personalized distributor landing pages." noIndex/><div className="mx-auto max-w-6xl">
    <header className="flex items-center gap-4"><Link to="/app" aria-label="Back to app home" className="grid h-11 w-11 place-items-center rounded-full border border-white/10 bg-white/[.04]"><ArrowLeft/></Link><div><p className="text-xs font-bold uppercase tracking-[.22em] text-cyan-300">Duplication tools</p><h1 className="text-2xl font-black">Share Center</h1></div></header>

    <section className="mt-7 grid gap-5 rounded-[28px] border border-white/10 bg-gradient-to-br from-cyan-400/[.1] to-blue-500/[.04] p-5 sm:grid-cols-[auto_1fr_auto] sm:items-center sm:p-7"><img src={distributor.avatar_url||'/icons/icon-192.png'} alt={distributor.display_name} className="h-24 w-24 rounded-3xl border border-white/15 bg-[#07132d] object-cover object-top"/><div><p className="text-sm text-cyan-200">Sharing as</p>{membership.role==='admin'?<select aria-label="Choose distributor profile" value={distributor.id} onChange={event=>{setSelectedDistributorId(event.target.value);setShowQr(false)}} className="mt-2 w-full max-w-sm rounded-xl border border-white/15 bg-[#07132d] px-4 py-3 text-lg font-black text-white"><option value={matchedDistributor?.id||''}>{matchedDistributor?.display_name}</option>{distributors.filter(item=>item.active&&item.id!==matchedDistributor?.id).map(item=><option key={item.id} value={item.id}>{item.display_name}</option>)}</select>:<h2 className="mt-1 text-3xl font-black">{distributor.display_name}</h2>}<p className="mt-2 text-sm text-slate-400">Every page keeps your referral attribution and contact details.</p></div><a href={`/d/${distributor.slug}`} target="_blank" rel="noreferrer" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-white/15 px-5 font-bold">View profile <ExternalLink className="h-4 w-4"/></a></section>
    ...
    <section className="mt-8 overflow-hidden rounded-[28px] border border-cyan-300/20 bg-white/[.035]"><div className="p-5 sm:p-7"><p className="text-xs font-bold uppercase tracking-[.2em] text-cyan-300">2 · Share {page.label}</p><h2 className="mt-2 text-2xl font-black">Ready for your next conversation</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">{page.description} The lead remains connected to {distributor.display_name} inside the True Legacy system.</p><p className="mt-5 break-all rounded-xl border border-white/10 bg-black/20 p-4 text-sm text-cyan-100">{url}</p><div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4"><button onClick={share} className="share-primary"><Share2/>Share now</button><button onClick={copy} className="share-secondary"><Copy/>{copied?'Copied':'Copy'}</button><button onClick={()=>setShowQr(value=>!value)} className="share-secondary"><QrCode/>QR code</button><a href={url} target="_blank" rel="noreferrer" className="share-secondary"><ExternalLink/>Preview</a></div></div>
      {showQr&&<div className="grid gap-6 border-t border-white/10 bg-black/20 p-5 sm:grid-cols-[220px_1fr] sm:items-center sm:p-7"><div className="rounded-2xl bg-white p-3"><img src={qrUrl} alt={`${page.label} QR code`} className="w-full"/></div><div><h3 className="text-xl font-black">Scan to open {page.label}</h3><p className="mt-2 text-sm leading-6 text-slate-400">Use this QR code at presentations, events, or in printed materials. It opens your selected personalized page.</p><a href={qrUrl} download={`${distributor.slug}-${page.id}-qr.png`} className="mt-4 inline-flex min-h-11 items-center rounded-xl bg-cyan-400 px-5 font-black text-slate-950">Download QR</a></div></div>}
    </section>
  </div></main>
}

function Message({title,body,action}:{title:string;body:string;action?:React.ReactNode}){return <main className="flex min-h-screen items-center justify-center bg-[#05091a] p-5 text-white"><div className="max-w-md text-center"><Sparkles className="mx-auto h-10 w-10 text-cyan-300"/><h1 className="mt-5 text-3xl font-black">{title}</h1><p className="mt-4 text-slate-400">{body}</p>{action&&<div className="mt-7">{action}</div>}</div></main>}
