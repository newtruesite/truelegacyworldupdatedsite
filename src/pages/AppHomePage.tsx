...
import { ArrowRight, Bell, BookOpenCheck, CalendarDays, CheckCircle2, Copy, ExternalLink, GraduationCap, LayoutDashboard, LifeBuoy, Link2, LogOut, QrCode, Share2, Sparkles, TrendingUp } from 'lucide-react'
...
export default function AppHomePage() {
  ...
  return <main className="min-h-screen bg-[#05091a] px-4 pb-28 pt-[max(24px,env(safe-area-inset-top))] text-white sm:px-6"><SEO title="True Legacy App" description="Private True Legacy distributor home." noIndex />
    <div className="mx-auto max-w-6xl">
      ...
      <section className="mt-7 grid grid-cols-2 gap-3 lg:grid-cols-4"><AppCard to="/crm" icon={<LayoutDashboard/>} eyebrow="Manage" title="Leads" detail={`${leads.length} accessible`} color="cyan"/><AppCard to="/crm/growth" icon={<TrendingUp/>} eyebrow="Duplicate" title="Growth" detail="Links & progress" color="amber"/><AppCard to="/training" icon={<GraduationCap/>} eyebrow="Learn" title="Academy" detail="Courses & modules" color="violet"/><AppCard to="/events" icon={<CalendarDays/>} eyebrow="Connect" title="Events" detail="Weekly live calls" color="emerald"/></section>
      ...
      <section className="mt-8 rounded-[28px] border border-blue-300/15 bg-gradient-to-r from-blue-500/[.09] to-cyan-400/[.04] p-5 sm:p-7"><div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-4"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-blue-400/10 text-blue-200"><LifeBuoy className="h-6 w-6"/></span><div><p className="text-xs font-bold uppercase tracking-[.2em] text-blue-200">Official Enagic resource</p><h2 className="mt-2 text-xl font-black">Distributor Support Portal</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">Access your Enagic distributor information and support tools using your separate Enagic credentials.</p></div></div><a href="https://information.enagic.com/login" target="_blank" rel="noopener noreferrer" className="inline-flex min-h-12 shrink-0 items-center justify-center gap-2 rounded-xl bg-blue-600 px-5 font-black text-white transition-colors hover:bg-blue-500">Open Enagic Portal <ExternalLink className="h-4 w-4"/></a></div></section>
    </div>
  </main>
}
