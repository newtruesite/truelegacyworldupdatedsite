      ...
      <main className="content-wrapper">
        ...
        <section className="relative overflow-hidden bg-[#060b1e] py-20 md:py-28">
          ...
          <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-4 sm:px-6 lg:grid-cols-2">
            <div><p className="text-xs font-semibold uppercase tracking-[.3em] text-tl-gold">{homeLocalized.duoLabel}</p><h2 className="mt-4 text-4xl font-black text-white md:text-6xl">{homeLocalized.duoTitle}<br/><span className="gradient-text">{homeLocalized.duoAccent}</span></h2><p className="mt-6 max-w-xl text-lg leading-8 text-slate-300">{homeLocalized.duoBody}</p><a href="https://youtu.be/lB5fW55DmaI" target="_blank" rel="noreferrer" onClick={()=>trackEvent('video_click',{video:'duo_demo',locale})} className="mt-8 inline-flex min-h-14 items-center justify-center gap-3 rounded-xl bg-gradient-to-r from-tl-gold to-amber-400 px-7 text-lg font-black text-slate-950 shadow-lg shadow-amber-500/20"><PlayCircle className="h-6 w-6"/>{homeLocalized.watchDuo}</a><br/><Link to="/distributors" className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-tl-gold">{homeLocalized.connectDistributor} <ArrowRight className="h-4 w-4"/></Link></div>
            <div className="relative mx-auto w-full max-w-xl"><div className="absolute inset-10 rounded-full bg-cyan-400/20 blur-3xl"/><div className="relative grid grid-cols-2 items-end gap-4 rounded-[2rem] border border-white/10 bg-white/[.04] p-5 sm:p-8"><div className="flex h-full flex-col justify-end"><img src="/products/k8.png" alt="K8 flagship Kangen Water ionizer" className="mx-auto max-h-[390px] w-full object-contain drop-shadow-2xl"/><p className="mt-4 text-center font-bold text-white">K8 Flagship</p><a href="https://youtu.be/1nkOCId-SfQ" target="_blank" rel="noreferrer" onClick={()=>trackEvent('video_click',{video:'water_demo',locale})} className="mt-3 inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-cyan-500 px-3 text-center text-xs font-bold text-slate-950 sm:text-sm"><PlayCircle className="h-4 w-4"/>{homeLocalized.waterDemo}</a></div><div className="flex h-full flex-col justify-end"><img src="/products/emguarde-go.png" alt="emGuarde GO product set" className="mx-auto max-h-[310px] w-full object-contain drop-shadow-2xl"/><p className="mt-4 text-center font-bold text-white">emGuarde GO</p><a href="https://youtu.be/5wuY1dKjHds" target="_blank" rel="noreferrer" onClick={()=>trackEvent('video_click',{video:'emguarde_go',locale})} className="mt-3 inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-cyan-300/30 bg-white/5 px-3 text-center text-xs font-bold text-white sm:text-sm"><PlayCircle className="h-4 w-4"/>emGuarde GO</a></div></div></div>
          </div>
        </section>
