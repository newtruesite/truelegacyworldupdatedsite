...
export default function AppLibraryPage() {
  ...
  return <main className="min-h-screen bg-[#05091a] px-4 pb-28 pt-6 text-white sm:px-6 lg:px-8">
    ...
    <div className="mx-auto max-w-7xl">
      ...
      <section className="mt-7 overflow-hidden rounded-[30px] border border-cyan-300/20 bg-[radial-gradient(circle_at_top_right,rgba(34,211,238,.18),transparent_35%),linear-gradient(145deg,rgba(37,99,235,.12),rgba(255,255,255,.025))] p-6 sm:p-9">
        <span className="inline-flex items-center gap-2 rounded-full border border-cyan-300/20 bg-cyan-300/[.07] px-3 py-1.5 text-xs font-bold text-cyan-200"><LockKeyhole className="h-3.5 w-3.5" /> True Legacy distributors only</span>
        <h2 className="mt-5 max-w-4xl text-3xl font-black sm:text-5xl">Everything here is built for True Legacy.</h2>
        <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-300 sm:text-base">Open the Academy, manage leads, grow your network, share official pages, and stay connected to the True Legacy community—all from one private workspace.</p>
        <div className="mt-7 flex items-start gap-3 rounded-2xl border border-amber-300/15 bg-amber-300/[.06] p-4 text-sm leading-6 text-slate-300"><Sparkles className="mt-0.5 h-5 w-5 shrink-0 text-amber-300" /><p><strong className="text-amber-200">True Legacy only:</strong> this center contains native True Legacy tools and destinations. External resource catalogs are kept separate so ownership and purpose remain clear.</p></div>
      </section>
      <section className="mt-9"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-amber-300">Your workspace</p><h2 className="mt-2 text-3xl font-black">Choose a True Legacy tool</h2><p className="mt-2 text-sm text-slate-500">Each destination is part of the True Legacy distributor experience.</p></div>
        <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{TRUE_LEGACY_TOOLS.map(tool => { const Icon = tool.icon; return <Link key={tool.to} to={tool.to} className="group flex min-h-64 flex-col rounded-[24px] border border-white/10 bg-white/[.035] p-5 transition hover:-translate-y-1 hover:border-cyan-300/30"><div className="flex items-start justify-between"><span className="grid h-12 w-12 place-items-center rounded-xl bg-cyan-400/10 text-cyan-300"><Icon className="h-6 w-6" /></span><span className="rounded-full border border-cyan-300/20 bg-cyan-300/[.08] px-2.5 py-1 text-[10px] font-black uppercase tracking-wider text-cyan-200">True Legacy</span></div><p className="mt-5 text-xs font-bold uppercase tracking-[.17em] text-slate-500">{tool.eyebrow}</p><h3 className="mt-2 text-xl font-black">{tool.title}</h3><p className="mt-3 text-sm leading-6 text-slate-400">{tool.detail}</p><div className="mt-auto flex items-center justify-between border-t border-white/[.07] pt-5"><span className="text-xs font-bold text-slate-500">Private workspace</span><span className="inline-flex items-center gap-2 text-sm font-black text-cyan-200">Open <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span></div></Link> })}</div>
      </section>
