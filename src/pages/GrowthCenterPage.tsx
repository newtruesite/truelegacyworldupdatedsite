...
export default function GrowthCenterPage() {
  ...
  if(loading)return <main className="min-h-screen bg-[#060b1e]"/>
  ...
  return <main className="min-h-screen bg-[#060b1e] px-4 py-8 text-white md:px-8"><SEO title="True Legacy Growth Center" description="Private distributor sharing and training progress center." noIndex/><div className="mx-auto max-w-7xl"><header className="flex flex-col gap-5 border-b border-white/10 pb-7 md:flex-row md:items-end md:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.25em] text-cyan-300">Duplication & team development</p><h1 className="mt-2 text-4xl font-black">Growth Center</h1><p className="mt-2 text-sm text-slate-400">Personal links, campaign results, onboarding, and training progress.</p></div><div className="flex gap-2"><Link to="/crm" className="rounded-xl border border-white/15 px-4 py-3 text-sm">Lead dashboard</Link><button onClick={()=>crmSupabase?.auth.signOut()} className="inline-flex items-center gap-2 rounded-xl border border-white/15 px-4 py-3 text-sm"><LogOut className="h-4 w-4"/>Sign out</button></div></header>
  {visible.length>1&&<section className="mt-6 rounded-2xl border border-white/10 bg-white/[.03] p-4"><label className="text-xs font-bold uppercase tracking-wider text-slate-500">Viewing distributor/team member</label><select value={distributor?.id||''} onChange={e=>setSelected(e.target.value)} className="mt-2 h-11 w-full max-w-md rounded-xl border border-white/10 bg-[#0a1020] px-4"><option value={membership.distributor_id||''}>My progress</option>{visible.filter(d=>d.id!==membership.distributor_id).map(d=><option key={d.id} value={d.id}>{d.display_name}</option>)}</select></section>}
