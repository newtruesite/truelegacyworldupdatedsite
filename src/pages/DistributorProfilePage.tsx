...
export default function DistributorProfilePage() {
    ...
    const handleScroll = () => {
      if (!ticking) {
        window.requestAnimationFrame(() => {
          ...
          setIsScrolled((prev) => {
            if (currentY > 340) return true
            if (currentY < 200) return false
            return prev
          })
          ticking = false
        })
  ...
  return (
    <div className="page-wrapper bg-black text-white selection:bg-cyan-500 selection:text-black">
      ...
      {/* STICKY COMPACT LEADER HEADER (Smooth Framer Motion slide-in on scroll) */}
      <AnimatePresence>
        {profile && isScrolled && (
          <motion.div
            ...
            className="fixed top-16 left-0 right-0 z-40 pointer-events-auto"
          >
            <div className="mx-auto max-w-6xl px-4 sm:px-6 py-2">
              <div className="flex items-center justify-between gap-3 rounded-2xl border border-white/15 bg-[#090d16]/95 p-3 shadow-[0_12px_40px_rgba(0,0,0,0.85)] backdrop-blur-xl">
                {/* Leader Avatar & Identity */}
                <div className="flex items-center gap-3 min-w-0">
                  <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-full border border-cyan-400/60 shadow-lg">
                    <img
                      src={leaderPhoto}
