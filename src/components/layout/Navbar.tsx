...
export function Navbar() {
    ...
    const navLabels = {
        ...
        events: locale === 'es' ? 'Eventos' : locale === 'fr' ? 'Événements' : locale === 'pt' ? 'Eventos' : 'Events',
        community: locale === 'es' ? 'Comunidad' : locale === 'fr' ? 'Communauté' : locale === 'pt' ? 'Comunidade' : 'Community',
        countries: locale === 'es' ? 'Países' : locale === 'fr' ? 'Pays' : locale === 'pt' ? 'Países' : 'Countries',
        products: locale === 'es' ? 'Productos' : locale === 'fr' ? 'Produits' : locale === 'pt' ? 'Produtos' : 'Products',
        distributors: locale === 'es' ? 'Distribuidores' : locale === 'fr' ? 'Distributeurs' : locale === 'pt' ? 'Distribuidores' : 'Distributors',
            ...
            >
                <nav className="mx-auto flex h-14 w-full max-w-7xl items-center justify-between px-4 sm:px-6">
                                    ...
                                    <div key={link.to} className="relative" ref={countriesRef}>
                                        ...
                                        <AnimatePresence>
                                            {countriesOpen && (
                                                ...
                                                >
                                                    <div className="p-2 grid grid-cols-1 gap-1" role="listbox">
                                                        {COUNTRIES.map((c) => (
                                                            <button
                                                                key={c.slug}
                                                                onClick={() => {
                                                            ...
                                                            >
                                                                <span className="inline-flex h-5 w-7 flex-shrink-0 overflow-hidden rounded border border-white/20 bg-[#0a2060]">
                                                                    {failedFlagSlugs.has(c.slug) ? (
                                                                        <span className="flex h-full w-full items-center justify-center text-[10px] leading-none">{c.flagEmoji}</span>
                                                                    ) : (
                                                                ...
                                                                <span className="font-medium text-left flex-1 break-words leading-tight">{c.name}</span>
                                                            </button>
                                                        ))}
                                                    </div>
                                                </motion.div>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        >
                            {country ? navLabels.unlockLegacy : (locale === 'es' ? 'Únete al Equipo' : locale === 'fr' ? "Rejoindre l'Équipe" : locale === 'pt' ? 'Junte-se à Equipe' : 'Join the Team')}
                        </a>

                        {/* Hamburger Button - Mobile Only */}
                        <button
                            onClick={() => setMenuOpen(!menuOpen)}
                            className="md:hidden inline-flex items-center justify-center p-2 rounded-md text-slate-300 hover:text-white hover:bg-white/5 transition-colors"
                ...
                <div className="flex w-full flex-col gap-y-4">
                    ...
                    <div className="space-y-1">
                        <span className="px-2 text-xs font-semibold uppercase tracking-wider text-slate-500">
                            {locale === 'es' ? 'Navegación' : locale === 'fr' ? 'Navigation' : locale === 'pt' ? 'Navegação' : 'Navigation'}
                        </span>
                        {mainNavLinks.map((link) => (
                            link.external ? (
                                <a
                                ...
                                <Link
                                    ...
                                    className={cn(
                                        ...
                                        pathname === link.to ? 'text-white bg-white/10' : 'text-slate-300 hover:text-white hover:bg-white/5'
                                    )}
                                >
                                    {link.label}
                                </Link>
                    ...
                    <div className="space-y-1">
                        <span className="px-2 text-xs font-semibold uppercase tracking-wider text-slate-500">
                            {locale === 'es' ? 'Productos' : locale === 'fr' ? 'Produits' : locale === 'pt' ? 'Produtos' : 'Products'}
                        </span>
                        ...
                        >
                            <IconShield className="text-purple-400 flex-shrink-0" /> {navLabels.navProductEmguarde}
                        </Link>
                        ...
                        >
                            <IconDroplets className="text-cyan-400 flex-shrink-0" /> {navLabels.navProductK8}
                        </Link>
                    </div>
                    ...
                    <div className="space-y-2">
                        <span className="px-2 text-xs font-semibold uppercase tracking-wider text-slate-500">
                            {locale === 'es' ? 'Países' : locale === 'fr' ? 'Pays' : locale === 'pt' ? 'Países' : 'Countries'}
                        </span>
                        <div className="grid grid-cols-2 gap-2 overflow-y-auto max-h-[50vh] min-h-0">
                            {COUNTRIES.map((c) => (
                                ...
                                >
                                    ...
                                    <span className="text-sm leading-tight text-left flex-1 break-words">{c.name}</span>
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="space-y-2">
                        <span className="px-2 text-xs font-semibold uppercase tracking-wider text-slate-500">
                            {locale === 'es' ? 'Idioma' : locale === 'fr' ? 'Langue' : locale === 'pt' ? 'Idioma' : 'Language'}
                        </span>
        ...
        <AnimatePresence>
            {open && (
                <motion.div
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="fixed top-14 right-0 bottom-0 left-0 z-40 flex flex-col overflow-hidden md:hidden"
                    style={{
